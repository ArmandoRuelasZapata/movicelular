...
const ReportsScreen = () => {
  ...
  // ── Guardamos las coordenadas reales para el mapa ─────────────────────────
  ...
  // ── Obtener ubicación — ahora guarda latitud/longitud además del texto ────
  const obtenerUbicacionActual = async () => {
    ...
    try {
      ...
      // Guardamos coordenadas numéricas para el mapa
  ...
  const procesarReporte = async () => {
    if (!tipoIncidencia || titulo.trim() === "" || descripcion.trim() === "" || recomendaciones.trim() === "") {
      setShowError(true);
      return;
    }
    ...
    try {
      ...
      const datosReporte = {
        titulo,
        descripcion,
        // Texto legible para mostrar en pantallas de moderador
        ubicacion:        location || "Ubicación no disponible",
        // ── Coordenadas numéricas para el mapa ──
        // El admin copia este documento a reportes_publicos → el mapa
        // lee latitude/longitude para colocar el marker exacto
      ...
      Alert.alert("Error", "No se pudo guardar el reporte. Revisa tu conexión a internet.");
    } finally {
      setSending(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TextInput
            ...
            editable={!sending}
          />
          <TouchableOpacity style={styles.imageUploadButton} onPress={tomarFoto} disabled={sending}>
            {image ? (
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.locationInputContainer}>
            ...
            />
            {loadingLocation && (
              <ActivityIndicator color={AppColors.PRIMARY} style={{ marginLeft: 10 }} />
            )}
          </View>
        </View>
