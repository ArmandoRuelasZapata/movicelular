...
const SUGERENCIAS = [
  { key: "Atención",   label: "Sugerir Revisión",  color: AppColors.PRIMARY, textColor: "#FFF"},
  { key: "Revisión",   label: "Sugerir Atención",  color: "#FFC107", textColor: "#000" },
  { key: "Finalizado", label: "Sugerir Cierre",     color: "#00C49A", textColor: "#FFF" },
] as const;
...
const getStatusColor = (estatus?: string): string => {
  switch (estatus?.toLowerCase()) {
    ...
    case "revision": case "revisión":       return "#FFC107";
    case "atencion": case "atención":       return AppColors.PRIMARY;
    default:                                return "#999";
  }
};
...
const getStatusLabel = (estatus?: string): string => {
  switch (estatus?.toLowerCase()) {
    ...
    case "revision": case "revisión":       return "En Revisión";
    case "atencion": case "atención":       return "En Atención";
...
const getActiveStep = (estatus?: string): number => {
  switch (estatus?.toLowerCase()) {
    case "atencion": case "atención":       return 2;
    case "revision": case "revisión":       return 1;
    ...
    <SafeAreaView style={[GlobalStyles.container, { backgroundColor: AppColors.PRIMARY }]}>
      ...
      <View style={[GlobalStyles.headerContainer, { borderBottomLeftRadius: 0, borderBottomRightRadius: 0 }]}>
        <View style={[GlobalStyles.profileHeader, { borderBottomLeftRadius: 0, borderBottomRightRadius: 0 }]}>
          <TouchableOpacity onPress={() => router.push('/(tabs)/ValidationScreen')} style={localStyles.backButton}>
      ...
      <View style={[GlobalStyles.menuContainer, GlobalStyles.menuItem]}>
        ...
        {loading && <ActivityIndicator size="large" color={AppColors.PRIMARY} style={{ marginTop: 40 }} />}
        {!loading && error && <Text style={localStyles.errorText}>{error}</Text>}
        ...
        {!loading && report && (
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
            ...
            <View style={[localStyles.statusBadge, { backgroundColor: getStatusColor(report.estatus) }]}>
              <Text style={localStyles.statusBadgeText}>{getStatusLabel(report.estatus)}</Text>
            </View>

            {/* Datos del reporte */}
            <View style={localStyles.detailsContainer}>
              <DetailItem label="Título:"            value={report.titulo} />
              <DetailItem label="Tipo de incidencia:" value={report.tipo_incidencia?.replace(/_/g, " ")} />
              <DetailItem label="Ubicación:"          value={report.ubicacion} />
              <DetailItem label="Descripción:"        value={report.descripcion} />

              {report.imagen?.trim() ? (
                <Image source={{ uri: report.imagen }} style={localStyles.reportImage} resizeMode="cover" />
              ) : (
                <View style={localStyles.noImageBox}>
                  <Ionicons name="image-outline" size={32} color="#CCC" />
                  <Text style={localStyles.noImageText}>Sin evidencia fotográfica</Text>
                </View>
              )}

              <DetailItem label="Recomendaciones:" value={report.recomendaciones} />
              {report.detalles_extra?.trim()
                ? <DetailItem label="Más detalles:" value={report.detalles_extra} />
                : null}
              <DetailItem label="Fecha de recepción:"  value={formatDate(report.created_at)} />
              <DetailItem label="Última actualización:" value={formatDate(report.updated_at)} />
            </View>
            ...
            <View style={localStyles.statusCard}>
              ...
              <View style={localStyles.stepperContainer}>
                ...
                <View style={localStyles.stepsRow}>
                  <StepNode label="En Revisión" active={getActiveStep(report.estatus) >= 1} />
                  <StepNode label="En Atención" active={getActiveStep(report.estatus) >= 2} />
                  <StepNode
                    label="Finalizado"
                    active={getActiveStep(report.estatus) >= 3}
                    color={getActiveStep(report.estatus) >= 3 ? "#00C49A" : undefined}
                  />
                </View>
              </View>
              ...
              <View style={localStyles.moderatorSection}>
                <Text style={localStyles.moderatorTitle}>
                  <Ionicons name="shield-checkmark-outline" size={13} color="#666" /> INFORMAR CAMBIO AL ADMINISTRADOR
                </Text>

                {updating ? (
                  <ActivityIndicator size="small" color={AppColors.PRIMARY} style={{ marginTop: 10 }} />
                  ...
                  <View style={localStyles.sentBox}>
                    <View style={localStyles.sentHeader}>
                      ...
                      <Text style={localStyles.sentTitle}>Propuesta enviada al administrador</Text>
                    </View>
                    <Text style={localStyles.sentDetail}>
                      Cambio sugerido:{" "}
                      <Text style={localStyles.sentBold}>"{report.mod_sugiere}"</Text>
                    </Text>
                    <Text style={localStyles.sentDetail} numberOfLines={2}>
                      Mensaje: <Text style={localStyles.sentItalic}>"{report.mod_mensaje}"</Text>
                    </Text>
                    <Text style={localStyles.sentWaiting}>
                      ⏳ Esperando acción del administrador...
                    </Text>
                    <TouchableOpacity style={localStyles.cancelBtn} onPress={cancelarSugerencia}>
                      <Ionicons name="close-circle-outline" size={15} color="#DC2626" />
                      <Text style={localStyles.cancelBtnText}>Retirar propuesta</Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={localStyles.modButtonsRow}>
                    {SUGERENCIAS.map((s) => (
                      <TouchableOpacity
                        key={s.key}
                        style={[localStyles.modBtn, { backgroundColor: s.color }]}
                        onPress={() => abrirModal(s.key)}
                      >
                        <Text style={[localStyles.modBtnText, { color: s.textColor }]}>{s.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                )}
            ...
            <Pressable onPress={(e) => e.stopPropagation()}>
              <View style={localStyles.modalCard}>
                ...
                <View style={localStyles.modalHeader}>
                  <Ionicons name="shield-checkmark" size={22} color={AppColors.PRIMARY} />
                  <Text style={localStyles.modalTitle}>Informe al Administrador</Text>
                ...
                <View style={localStyles.modalPillRow}>
                  <Text style={localStyles.modalPillLabel}>Cambio sugerido:</Text>
                  <View style={[
                    localStyles.modalPill,
                    { backgroundColor: SUGERENCIAS.find(s => s.key === sugerenciaSeleccionada)?.color ?? "#999" }
                  ]}>
                    <Text style={[
                      localStyles.modalPillText,
                      { color: SUGERENCIAS.find(s => s.key === sugerenciaSeleccionada)?.textColor ?? "#FFF" }
                    ]}>
                ...
                <Text style={localStyles.modalInputLabel}>
                  Motivo / observación del moderador <Text style={{ color: "#DC2626" }}>*</Text>
                </Text>
                <TextInput
                  ref={inputRef}
                  style={[localStyles.modalInput, mensajeError ? localStyles.modalInputError : null]}
                  ...
                  maxLength={300}
                  value={mensajeTexto}
                  onChangeText={(t) => { setMensajeTexto(t); setMensajeError(""); }}
                  returnKeyType="done"
                />
                {mensajeError ? (
                  <Text style={localStyles.modalErrorText}>
                    <Ionicons name="alert-circle-outline" size={12} /> {mensajeError}
                  </Text>
                ) : null}
                <Text style={localStyles.charCount}>{mensajeTexto.length}/300</Text>
                ...
                <View style={localStyles.modalBtnsRow}>
                  ...
                  >
                    ...
                    <Text style={localStyles.modalBtnSendText}>Enviar Informe</Text>
                  </TouchableOpacity>
                </View>
...
const DetailItem = ({ label, value }: DetailItemProps) => (
  <View style={localStyles.itemBox}>
    ...
    <Text style={[GlobalStyles.textBase, value ? localStyles.value : localStyles.placeholder]}>
      {value ?? "Sin información"}
    </Text>
  </View>
);

interface StepNodeProps { label: string; active?: boolean; color?: string; }
...
const localStyles = StyleSheet.create({
  ...
  reportIdText:    { fontSize: 18, fontWeight: "bold", color: "#000", marginBottom: 10 },
  statusBadge:     { alignSelf: "flex-start", borderRadius: 20, paddingVertical: 5, paddingHorizontal: 15, marginBottom: 20 },
  statusBadgeText: { color: "#FFF", fontWeight: "bold", fontSize: 13 },
  detailsContainer:{ marginBottom: 10 },
  itemBox:         { marginBottom: 12 },
  label:           { fontSize: 13, fontWeight: "bold", color: "#000", marginBottom: 2, textTransform: "uppercase" },
  value:           { fontSize: 15, color: "#333" },
  placeholder:     { fontSize: 14, color: "#999" },
  reportImage:     { width: "100%", height: 200, borderRadius: 12, marginVertical: 10 },
  noImageBox:      { alignItems: "center", paddingVertical: 20, backgroundColor: "#F5F5F5", borderRadius: 12, marginVertical: 10 },
  noImageText:     { color: "#CCC", marginTop: 6, fontSize: 13 },

  // Stepper
  statusCard:       { borderWidth: 1, borderColor: "#333", borderRadius: 25, paddingVertical: 30, paddingHorizontal: 15, backgroundColor: "#FFF", alignItems: "center", marginTop: 1, marginBottom: 100 },
  statusTitle:      { fontSize: 20, fontWeight: "bold", marginBottom: 40, color: "#000" },
  stepperContainer: { width: "100%", position: "relative", justifyContent: "center" },
  progressLine:     { position: "absolute", top: 15, left: "10%", right: "10%", height: 1.5, backgroundColor: "#333" },
  ...
  // Sección moderador
  moderatorSection: { marginTop: 40, width: "100%", alignItems: "center", borderTopWidth: 1, borderTopColor: "#EEE", paddingTop: 20 },
  moderatorTitle:   { fontSize: 12, fontWeight: "bold", color: "#666", marginBottom: 15 },
  modButtonsRow:    { flexDirection: "row", justifyContent: "space-between", width: "100%" },
  modBtn:           { paddingVertical: 10, paddingHorizontal: 5, borderRadius: 10, width: "31%", alignItems: "center" },
  ...
  sentHeader:  { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 10 },
  ...
  sentWaiting: { fontSize: 12, color: "#6B7280", marginTop: 10, marginBottom: 12 },
  cancelBtn:   { flexDirection: "row", alignItems: "center", gap: 5, alignSelf: "center", paddingVertical: 6, paddingHorizontal: 14, borderRadius: 8, borderWidth: 1.5, borderColor: "#DC2626" },
  ...
  modalPillRow:   { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 18 },
  ...
  modalInputLabel:{ fontSize: 13, fontWeight: "600", color: "#333", marginBottom: 8 },
  modalInput: {
  ...
  modalBtn:       { flex: 1, paddingVertical: 14, borderRadius: 12, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 7 },
  modalBtnCancel: { borderWidth: 1.5, borderColor: "#D1D5DB", backgroundColor: "#F9FAFB" },
